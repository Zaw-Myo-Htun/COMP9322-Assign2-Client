...
public class InfoYesNoCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response)
		...
		WebResource webResource = client
				.resource("http://localhost:8080/HelloWorldCxfRest/foundIT/updateResult/"
						+ request.getParameter("jobAppID")
						+ "/"
						+ request.getSession().getAttribute("reviewerID")
								.toString()
						+ "/"
		...
		if (r1.getStatus() != 204) {
			...
			RequestDispatcher rd = request
					.getRequestDispatcher("/control?action=ToReviewerHomePage");
